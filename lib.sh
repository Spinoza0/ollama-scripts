...
# -----------------------------------------------------------------------------
# Получить список моделей из локального хранилища (без запущенного сервера)
# Возвращает: список моделей вида "имя/тег" (по одной на строку)
# -----------------------------------------------------------------------------
ollama_list_models_storage() {
    local base="$HOME/.ollama/models/manifests/registry.ollama.ai/library"
    if [ ! -d "$base" ]; then
        return 1
    fi
    find "$base" -type f ! -name ".DS_Store" 2>/dev/null | sed "s|^$base/||"
}
